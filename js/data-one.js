...
function fm(){
	...
	for(var i=0;i<liss.length;i++){
		...
		liss[i].addEventListener('click',function(){
			console.log(1);
			window.open('commodity-classification.html#'+this.ind+'','_blank');
		})
	}
	//添加事件
	...
	// 		window.open('commodity-classification.html#'+ev.target.getAttribute('index')+'','_blank');
	// 	}
	// })
	...
	u.onmouseenter=function(){
		...
		for(var i=0;i<lis.length;i++){
			...
			lis[i].onmouseenter=function(){
				...
				box.innerHTML=strr;
				var thisNum = this.indexd
				//添加完毕后获取li并进行点击和跳转
				var lL=box.querySelectorAll('li');
				//给所有的lL添加点击事件进行跳转
				for(var i=0;i<lL.length;i++){
					lL[i].zdy=i;
					lL[i].addEventListener('click',function(){
						window.open('commodity-classification.html#'+thisNum+'+'+this.zdy+'','_blank');
					})
				}
...
/* 优品专辑数据渲染 */
function fx(){
	var titleNeirong=document.querySelector('.title-neirong');
	
	var contentContent=document.querySelector('.content-content');
	var ul=contentContent.querySelector('ul');
	...
	for(var st in brand){
		str +='<li>'+
				'<a href="">'+
					'<span class="'+brand[st].clss+'"></span>'+st+
				'</a>'+
				'<span></span>'+
			'</li>'
	}
	//将str赋值给ul
	titleNeirong.innerHTML=str;
	var lis=titleNeirong.querySelectorAll('li');
	//默认渲染第一个
	lis[0].style.color='blue';
	lis[0].style.borderBottom='none';
	lis[0].style.borderBottom='2px solid blue';
	BIGIMG.innerHTML='<a href="">'+
						'<img src="'+brand['洗衣机'].bigimg+'" >'+
					'</a>'
	for(var i=0;i<brand['洗衣机'].title.length;i++){
		sdr +='<li>'+
				'<a href="">'+
					'<span title="'+brand['洗衣机'].title[i]+'">'+brand['洗衣机'].title[i]+'</span>'+
				'</a>'+
				'<span>'+brand['洗衣机'].price[i]+'</span>'+
				'<a href="">'+
					'<img src="'+brand['洗衣机'].img[i]+'" >'+
				'</a>'+
			'</li>'
	}
	ul.innerHTML=sdr;
	...
	for(var i=0;i<lis.length;i++){
		...
		lis[i].onmouseenter=function(){
			...
			var shuju=nodeToString(a);
			
			BIGIMG.innerHTML='<a href="">'+
								'<img src="'+brand[shuju].bigimg+'" >'+
							'</a>'
			for(var k=0;k<brand[shuju].title.length;k++){
				sdr +='<li>'+
						'<a href="javascript:;">'+
							'<span title="'+brand[shuju].title[k]+'">'+brand[shuju].title[k]+'</span>'+
						'</a>'+
						'<span>'+brand[shuju].price[k]+'</span>'+
						'<a href="javascript:;">'+
							'<img src="'+brand[shuju].img[k]+'" >'+
						'</a>'+
					  '</li>'
			}
			ul.innerHTML=sdr;
			//添加完毕后获取li并给li添加点击事件       点击开启新窗口
			var shujuShu=document.getElementById('shuju');
			var liLi=shujuShu.querySelectorAll('li');
			for(var i=0;i<liLi.length;i++){
				liLi[i].onclick=function(){
					//获取图片的src和
					//开启一个新窗口
					window.open('Specific commodities.html');
				}
			}
